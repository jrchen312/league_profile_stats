...
class App(object):
    ...
    @_safeMethod
    def _mouseWheelWrapper(app, event):
        if (not app._running) or app._paused: return
        if (app._methodIsOverridden('mouseWheel')):
            print('ok')
...
class TopLevelApp(App):
    ...
    def mouseWheel(app, event): app.callFn('mouseWheel', app, event)
...
class ModalApp(App):
    ...
    def mouseWheel(app, event):
        if (app._activeMode != None): app._activeMode.mouseWheel(event)
    def timerFired(app):
